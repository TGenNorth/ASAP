...
<xsl:text/>Sample<xsl:for-each select="sample[1]"><xsl:for-each select="assay[not(@type='presence/absence')]"><xsl:choose><xsl:when test="@type = 'SNP'"><xsl:for-each select="amplicon//snp/@name[. != 'unknown' and . != 'position of interest']"><xsl:sort select="."/><xsl:text>&#x9;</xsl:text><xsl:value-of select="ancestor::assay/@name"/>/<xsl:value-of select="."/></xsl:for-each></xsl:when><xsl:when test="@type = 'ROI'"><xsl:for-each select="amplicon//region_of_interest/@name"><xsl:sort select="."/><xsl:text>&#x9;</xsl:text><xsl:value-of select="ancestor::assay/@name"/>/<xsl:value-of select="."/></xsl:for-each></xsl:when></xsl:choose></xsl:for-each></xsl:for-each>
<xsl:text>&#xa;</xsl:text>
...
<xsl:text>&#x9;</xsl:text><xsl:value-of select="../@depth"/></xsl:for-each></xsl:when><xsl:when test="@type = 'ROI'"><xsl:for-each select="amplicon//region_of_interest/@name"><xsl:sort select="."/><xsl:text>&#x9;</xsl:text><xsl:value-of select="../@depth"/></xsl:for-each></xsl:when></xsl:choose></xsl:for-each>
<xsl:text>&#xa;</xsl:text>
</xsl:for-each>
<xsl:text/>
</xsl:template>
