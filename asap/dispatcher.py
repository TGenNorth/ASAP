...
def processBam(sample_name, json_file, bam_file, xml_dir, dependency, depth, breadth, proportion, percid, mutdepth, primer_mask_file, wiggle, pmaskbam, ponlybam, base_qual, smor=False, wholegenome=False, debug=False, allele_min_reads=8, con_prop=0.8, fill_gap_char=None, fill_del_char=None):
    import os
    ...
    command = "bamProcessor -j %s -b %s -o %s -d %d --breadth %f -p %f -m %d -i %f%s%s%s%s%s --allele-output-threshold %d --primer-mask %s --primer-wiggle %i --primer-mask-bam %s --primer-only-bam %s --min_base_qual %i --consensus-proportion %f" % (json_file, bam_file, out_file, depth, breadth, proportion, mutdepth, percid, smor_option, wholegenome_option, debug_option, fill_gap_option, mark_del_option, allele_min_reads, primer_mask_file, wiggle, pmaskbam, ponlybam, base_qual, con_prop)
    jobid = _submit_job(job_manager, command, job_params, (dependency,)) if dependency else _submit_job(job_manager, command, job_params)
