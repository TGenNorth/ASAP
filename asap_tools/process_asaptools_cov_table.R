...
if (is.na(poi_csv) || poi_csv == "NULL" || poi_csv == "") {
  ...
  Gene_Positions <- data.frame(
    position = seq(genome_min, genome_max),
    gene = "Whole_Genome"
  )
} else {
...
# 3. Join with your data and calculate coverage
# We join 'final_array' with our 'Gene_Positions' map
# 
array_info_gene <- left_join(final_array, Gene_Positions, by = "position") %>%
  filter(!is.na(gene)) # Only keep positions that fall within our defined ranges

Amplicon_Coverage <- array_info_gene %>%
  group_by(name, gene) %>%
  summarise(
    total_bp = n(),
  ...
  select(name, gene, Coverage) %>%
  pivot_wider(names_from = gene, values_from = Coverage)
