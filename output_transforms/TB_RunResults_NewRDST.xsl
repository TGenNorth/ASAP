	    		...
	    		<xsl:for-each select="assay[not(@type='presence/absence')]">
	    		    ...
	    		    <xsl:choose>
	    		        ...
	    		        <xsl:when test="@type = 'ROI'">
                                        <xsl:choose><xsl:when test="@name = 'pncA'">
                                            <xsl:value-of select="@name"/> non-synonymous SNPs (% res)</xsl:when>
                                        <xsl:otherwise>
                                            <xsl:value-of select="@name"/> codon (% res)</xsl:otherwise>
                                        </xsl:choose></xsl:when>
                    ...
                    <tr>
			    		...
			    		<xsl:for-each select="assay[not(@type='presence/absence')]">
			    		    <td align="center"><xsl:for-each select="amplicon">
			    		        <xsl:choose>
                                                    <xsl:when test="../@name = 'pncA'">
                                                        <xsl:variable name="PRINT_SOMETHING">
                                                        <xsl:if test="not(.//amino_acid_sequence/@aa_changes &gt; 0)"><em>none</em></xsl:if>
			    		                <xsl:for-each select="region_of_interest[not(significance/@flag)]">
                                                            <xsl:for-each select="amino_acid_sequence">
			    		                        <xsl:if test="@aa_changes &gt; 0">
			    		                            <xsl:value-of select="../@name"/> - <xsl:value-of select="@aa_changes"/> (<xsl:value-of select='format-number(@percent, "##.##")'/>%)<br/>
			    		                        </xsl:if>
                                                            </xsl:for-each>    
			    		                </xsl:for-each>
                                                        </xsl:variable>
                                                        <xsl:copy-of select="$PRINT_SOMETHING"/>
                                                        <xsl:if test="$PRINT_SOMETHING = ''">
                                                            <em>only low coverage mutations</em>
                                                        </xsl:if>
                                                    </xsl:when>
	    		...
	    		<xsl:for-each select="assay[not(@type='presence/absence')]">
	    		    <td align="center"><xsl:for-each select="amplicon">
	    		        <xsl:choose>
			    	        ...
			    	        <xsl:for-each select="region_of_interest">
                                             <xsl:for-each select="amino_acid_sequence">
			    		         <xsl:if test="@aa_changes &gt; 0 and not(../significance/@flag)">
			    		             <xsl:value-of select="../@name"/> - <xsl:value-of select="@aa_changes"/> (<xsl:value-of select='format-number(@percent, "##.##")'/>%)<br/>
			    		         </xsl:if>
                                             </xsl:for-each>    
			    		</xsl:for-each>
...
<!-- Per Sample Detailed Results -->
	<exsl:document method="html" href="{/analysis/@run_name}/{@name}_details.html">
	    ...
	    <body>
	    	...
	    	<br />
	    	<br />
	    	<h3><a href="#pncA-graph">pncA</a> assays for sample: <xsl:value-of select="@name"/></h3>
	    	<table border="2" width="100%">
	    		<tr>
	    		<th>Region Name</th>
	    		<th>Region Position</th>
	    		<th># Reads</th>
	    		<th>Amino Acid Sequence - # AA changes - # reads containing mutation(% reads)</th>
                        <th>Significance</th>
	    		<th>SNPs found(% reads containing SNP)</th>
	    		</tr>
	    		<xsl:for-each select="assay[@name = 'pncA']">
	    		    ...
	    		    <tr>
	    		        ...
	    		        <td><xsl:value-of select="@depth"/></td>
                                <td><xsl:for-each select="amino_acid_sequence">
			    	    <xsl:value-of select="."/> - <xsl:value-of select="@aa_changes"/> - <xsl:value-of select="@count"/>(<xsl:value-of select='format-number(@percent, "##.##")'/>%)
                                    <br/>
                                </xsl:for-each></td>
	    		        <td><xsl:value-of select="significance"/><xsl:if test="significance/@flag">(<xsl:value-of select="significance/@flag"/>)</xsl:if></td>
	    	...
	    	<br />
	    	<h3>Other drug resistance assays for sample: <xsl:value-of select="@name"/></h3>
	    	<table border="2" width="100%">
	    		<tr>
	    		<th>Assay Name</th>
