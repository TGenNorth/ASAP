...
def _primer_mask(primer_file, bam_file_name, wiggle, pmaskbam, ponlybam, smor):
    ...
    logging.info("Wrote primer masking alignment only file: %s" % bam_file_out)
    return bam_file_out
...
def _process_pileup(pileup, amplicon, depth, proportion, mutdepth, offset, wholegenome, smor, base_qual, con_prop, fill_gap_char, fill_del_char):
    ...
    if smor:
        smor_discard_array = [0] * amplicon_length
        overlap_discard_array = [0] * amplicon_length
        ...
        if smor:
            ...
            for read, pair in pairwise(reads):
                if read.alignment.query_name != pair.alignment.query_name:#keep pairs together
                    overlap_discard_array[pileupcolumn.pos] += 1
                    continue
                end_count = end_count + 1
                alignment = read.alignment
                try:
                    if read.is_del and pair.is_del:
                        #If this position is a deletion, it won't have a qscore, so lets use the average qscore of the aligned
                        # reads flanking this deletion
                        ...
                        if pair.indel > 0 and read.indel > 0: #This means the next position is an insertion
                            ...
                            if pair.alignment.query_sequence[pstart:pend] == alignment.query_sequence[start:end]:
                                ...
                                smor_discard_array[pileupcolumn.pos] += 1
    ...
    if smor:
        pileup_dict['smor_discards'] = ",".join(str(n) for n in smor_discard_array)
        pileup_dict['nonoverlap_discards'] = ",".join(str(n) for n in overlap_discard_array)
...
def _verify_percent_identity(samdata, ref_name, amplicon, percid, merge):
    ...
    seq_counter = Counter()
    aligned_reads = []
    logging.info("verify_percent_identity :: Checking %s for amplicon %s, length %i" % (samdata.filename, ref_name, amp_length))
    if merge:
        from operator import attrgetter
    ...
    for read in aligned_reads:
        if read.is_unmapped:
            logging.info("\tRead is unmapped, skipping....");
            continue
...
# ''' % (program_shortdesc, str(__date__))
    ...
    try:
        ...
        # parser_bamProcessor = subparsers.add_parser('bamProcessor', help='bamProcessor help')
        ...
        if percidlist and len(percidlist)%2 > 0:
            raise ValueError("identity list for percentages is not even and thus is incomplete")
            pass
        keep = args.keep
        merge = args.merge
        smor = args.smor
        ...
        if smor:
            if proportion > 0.0:
                smartSMOR = False #Turn off smartSMOR if user explicitely sets proportion flag
            else:
        ...
        if percidlist:
            sample_dict['identity_filter_list'] = dispatcher.listToString(percidlist)
        ...
        if primer_mask_file != 'False' and not wholegenome: #its been converted to a string from the analyzeAmplicons input
            masked_bam_file = _primer_mask(primer_file=primer_mask_file, bam_file_name=str(bam_fp.name), wiggle=wiggle, pmaskbam=pmaskbam, ponlybam=ponlybam, smor=smor)
            samdata = pysam.AlignmentFile(masked_bam_file, "rb")
            bam_file = masked_bam_file #Make sure we use the new bam from this point forward
        ...
        for assay in assay_list:
            ...
            for amplicon in assay.target.amplicons:
                ...
                amplicon_dict = {}
                seq_counter = None
                # Define the mostly likely desired percentage identity
                idToUse = 0
                if percid: # Identity given via flag
                    # logging.info("===PERCID===")
                    idToUse = percid
                if assay.percid: # JSON given General identity
                    idToUse = assay.percid
                    pass
                if amplicon.percid or percidlist: # JSON given Specific identity
                    if amplicon.percid:
                        idToUse = amplicon.percid
                    ##OVERWRITE JSON given if there is a matching flag given id
                    if percidlist:
                        logging.info("Identity List Given: "+str(percidlist))
                        for ampliconItem in percidlist:
                            if str(ampliconItem).lower().strip() == str(ref_name).lower().strip():
                                logging.info("Found percidlist amplicon: "+str(ampliconItem))
                                try:
                                    idToUse = percidlist[percidlist.index(ampliconItem)+1]
                                    idToUse = float(idToUse)
                                    pass
                                except Exception as e:
                                    logging.error("ERR in bamProcessor.py from percidlist arg parsing: "+str(e))
                                pass
                            pass
                        pass
                    pass
                ##USE identity
                if idToUse and idToUse > 0:
                    logging.info("Using Identity: "+str(idToUse))
                    (temp_file, discarded_reads, seq_counter) = _verify_percent_identity(samdata, ref_name, amplicon, idToUse, merge)
                    samdata = pysam.AlignmentFile(temp_file, "rb")
                    amplicon_dict['discarded_reads'] = str(discarded_reads)
                    logging.info("identity of "+str(idToUse)+" finished for "+str(ref_name))
                    pass
                    logging.info("===PERCID END===")
                elif samdata.closed:
