	...
	<exsl:document method="html" href="{/analysis/@run_name}/{@name}.html">
	    ...
	    <body>
	    	...
	    	<table border="2" width="100%">
	    		<tr>
	    		<th>Assay Name</th>
	    		<th># of Individual Reads</th>
                        <th>Average SMOR Depth(pairs)</th>
	    		<th>Coverage Breadth</th>
	    		<th>Significance</th>
	    		<th>SNPs found - pairs/total(%)</th>
	    		</tr>
	    		<xsl:for-each select="assay">
	    	...
	    	<table border="2" width="100%">
	    		...
	    		<th># of Individual Reads</th>
                        <th>Average SMOR Depth(pairs)</th>
	    		<th>Coverage Breadth</th>
	    		<th>Known SNPs - pairs/total(%) - Significance</th>
	    		<th>Other SNPs found - pairs/total(%)</th>
	    		</tr>
	    		<xsl:for-each select="assay">
	    		        ...
	    		        <xsl:if test="amplicon/@reads &gt; 0">
		    		        ...
		    		        <td><xsl:for-each select="amplicon/snp">
		    		            <xsl:if test="./@name != 'unknown'">
		    		                <xsl:value-of select="./@position"/><xsl:value-of select="./@reference"/>-><xsl:value-of select="./snp_call"/> - <xsl:value-of select="snp_call/@count"/>/<xsl:value-of select="@depth"/>(<xsl:value-of select='format-number(./snp_call/@percent, "##.##")'/>%)
	    	...
	    	<table border="2" width="100%">
	    		...
	    		<th># of Individual Reads</th>
                        <th>Average SMOR Depth(pairs)</th>
	    		<th>Coverage Breadth</th>
	    		<th>Region - Reference - Most common sequence - pairs/total(%) - Significance (# of aa changes)</th>
	    		<th>SNPs found - pairs/total(%)</th>
	    		</tr>
	    		<xsl:for-each select="assay">
	    		    ...
	    		    <tr>
	    		        ...
	    		        <xsl:if test="amplicon/@reads &gt; 0">
		    		        ...
		    		        <td><xsl:for-each select="amplicon/region_of_interest">
		    		            <xsl:value-of select="@region"/> - <xsl:value-of select="@reference"/> - <xsl:value-of select="amino_acid_sequence"/> - <xsl:value-of select="amino_acid_sequence/@count"/>/<xsl:value-of select="@depth"/>(<xsl:value-of select='format-number(amino_acid_sequence/@percent, "##.##")'/>%)
