        ...
        <body>
            ...
            <table border="1" cellpadding="3">
	    		...
	    		<xsl:for-each select="assay[not(@type='presence/absence')]">
	    		    ...
	    		    <xsl:choose>
	    		        <xsl:when test="@type = 'SNP'">
	    		            <xsl:for-each select=".//snp[not(@name='unknown') and not(@position=preceding-sibling::snp/@position)]">
	    		                <th nowrap="true"><xsl:value-of select="$assay_name"/><xsl:text disable-output-escaping="yes"><![CDATA[&nbsp;]]></xsl:text><xsl:value-of select="substring(@name, 1, string-length(@name) - 1)"/></th>
	    		            </xsl:for-each>
	    		        </xsl:when>
	    		        <xsl:when test="@type = 'ROI'">
	    		            <xsl:for-each select=".//region_of_interest">
	    		                <th nowrap="true"><xsl:value-of select="$assay_name"/><xsl:text disable-output-escaping="yes"><![CDATA[&nbsp;]]></xsl:text><xsl:value-of select="substring(mutation[1]/@name, 1, string-length(mutation[1]/@name) - 3)"/></th>
	    		            </xsl:for-each>
	    		        </xsl:when>
                ...
                <xsl:for-each select="sample">
                    ...
                    <tr>
                        <td nowrap="true"><a href="{/analysis/@run_name}/{./@name}_details.html"><xsl:value-of select="@name"/></a></td>
			    		<xsl:for-each select="assay[not(@type='presence/absence')]">
			    		    <xsl:choose>
			    		        <xsl:when test="@type = 'SNP'">
			    		            <xsl:for-each select=".//snp[not(@name='unknown') and not(@position=preceding-sibling::snp/@position)]">
			    		                <xsl:variable name="total" select="@depth"/>
			    		                <xsl:variable name="position" select="@position"/>
			    		                <xsl:variable name="resistant" select="sum(parent::amplicon//snp[not(@name='unknown') and @position=$position]//snp_call/@count)"/>
			    		                <td><xsl:value-of select='format-number(($total - $resistant) div $total * 100, "##.##")'/></td>
			    		            </xsl:for-each>
			    		        </xsl:when>
			    		        <xsl:when test="@type = 'ROI'">
			    		            <xsl:for-each select=".//region_of_interest">
			    		                <xsl:variable name="total" select="@depth"/>
			    		                <xsl:variable name="resistant" select="sum(.//mutation/@count)"/>
			    		                <td><xsl:value-of select="format-number(($total - $resistant) div $total * 100, '##.##')"/></td>
			    		            </xsl:for-each>
			    		        </xsl:when>
			    		        <xsl:otherwise><xsl:value-of select="@name"/></xsl:otherwise>
	    	...
	    	<em>Numbers indicate the percentage of the sample that is susceptible (i.e. does not contain a known resistant mutation at that locus).</em>
	    	<br />
        </body>
        </html>
